...
const (
	...
	ChemsLEFT_KEY          = 31
	ChemsRIGHT_KEY         = 32
	ChemsLEFT_BRACKET      = 33
	ChemsRIGHT_BRACKET     = 34
	ChemsEQUEAL_EQUAL      = 35
	ChemsNOTEQUAL          = 36
	ChemsGREATER_THAN      = 37
	ChemsLESS_THAN         = 38
	ChemsGREATER_EQUALTHAN = 39
	ChemsLESS_EQUEALTHAN   = 40
	ChemsMUL               = 41
	ChemsDIV               = 42
	ChemsMOD               = 43
	ChemsADD               = 44
...
const (
	...
	ChemsRULE_expr_rel     = 15
	ChemsRULE_expr_arit    = 16
	ChemsRULE_data_type    = 17
	ChemsRULE_expr_valor   = 18
)
...
func (s *HeadersContext) RINCLUDE() antlr.TerminalNode {
	return s.GetToken(ChemsRINCLUDE, 0)
}
...
func (s *HeadersContext) AllRFLOAT() []antlr.TerminalNode {
	return s.GetTokens(ChemsRFLOAT)
}

func (s *HeadersContext) RFLOAT(i int) antlr.TerminalNode {
	return s.GetToken(ChemsRFLOAT, i)
}
...
func (s *List_tempsContext) RFLOAT() antlr.TerminalNode {
	return s.GetToken(ChemsRFLOAT, 0)
}
...
func (p *Chems) Instructions() (localctx IInstructionsContext) {
	...
	for ((_la)&-(0x1f+1)) == 0 && ((1<<uint(_la))&((1<<ChemsRHEAP)|(1<<ChemsRSTACK)|(1<<ChemsRIF)|(1<<ChemsRGOTO)|(1<<ChemsRPSTACK)|(1<<ChemsRPHEAP)|(1<<ChemsINTEGER)|(1<<ChemsFLOAT)|(1<<ChemsID))) != 0 {
		{
			p.SetState(105)
...
func (p *Chems) expr_rel(_p int) (localctx IExpr_relContext) {
	...
	var _ antlr.ParserRuleContext = _prevctx // TODO: To prevent unused variable warning.
	_startState := 30
	p.EnterRecursionRule(localctx, 30, ChemsRULE_expr_rel, _p)
	var _la int
	...
	for _alt != 2 && _alt != antlr.ATNInvalidAltNumber {
		if _alt == 1 {
			...
			}
			{
				...
				if !(((_la-35)&-(0x1f+1)) == 0 && ((1<<uint((_la-35)))&((1<<(ChemsEQUEAL_EQUAL-35))|(1<<(ChemsNOTEQUAL-35))|(1<<(ChemsGREATER_THAN-35))|(1<<(ChemsLESS_THAN-35))|(1<<(ChemsGREATER_EQUALTHAN-35))|(1<<(ChemsLESS_EQUEALTHAN-35)))) != 0) {
...
func (p *Chems) expr_arit(_p int) (localctx IExpr_aritContext) {
	...
	var _ antlr.ParserRuleContext = _prevctx // TODO: To prevent unused variable warning.
	_startState := 32
	p.EnterRecursionRule(localctx, 32, ChemsRULE_expr_arit, _p)
	var _la int
	...
	case ChemsRPSTACK, ChemsRPHEAP, ChemsINTEGER, ChemsFLOAT, ChemsID:
		{
			p.SetState(200)
...
func (p *Chems) Data_type() (localctx IData_typeContext) {
	...
	p.EnterRule(localctx, 34, ChemsRULE_data_type)
...
func (p *Chems) Expr_valor() (localctx IExpr_valorContext) {
	...
	p.EnterRule(localctx, 36, ChemsRULE_expr_valor)
